...
#' calc(PropExpoRating, "MeanLoss", Limit = c(50, 200), Deductible = c(50, 100))
#' calc(PropExpoRating, "MeanLoss", c("50 xs 50", "200 xs 100"))
#'
#' ## CDF
#' calc(PropExpoRating, "CDF", mseq(50, 250, n = 200))
#' @export
...
#' @export
#' @usage NULL
calc.ExposureRating <- function(x, method = c("MeanLoss", "CDF"), ...) {
  method <- match.arg(method)
  .f <- double_dispatch("calc.ExposureRating", method)
  .f(x, ...)
}
...
#' @usage
#' ## Loss in Layer
#' calc(ExposureRating, "MeanLoss", ...)
calc.ExposureRating.MeanLoss <- function(x, ...) {
...
#' @rdname ExposureRating
#' @usage
#' ## CDF for Severity and Frequency
#' calc(ExposureRating, "CDF", q)
calc.ExposureRating.CDF <- function(x, q = NULL) {
  qMin <- min(q)
  ...
  # Calculation
  temp_Count <- calc.ExposureRating.MeanLoss(x, Limit = Arr_Limit, Deductible = Arr_LossAmount)$ExpectedLayerCount
