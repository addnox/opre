...
ct_Reinstatement <- function(Term, Limit) {
  ...
  f <- function(Loss) {
    ...
    ReinsFactor <- vapply(
      Loss,
      function(x) Reinstat[, sum(pmin(Upper - Lower, pmax(0, x - Lower)) * Percentage) / Limit],
      numeric(1L)
    )

    list(Loss = LossToLayer, ReinstatementFactor = ReinsFactor)
  }
}
...
XL <- function(x = NULL, Limit = Inf, Deductible = 0, AAL = Inf, AAD = 0, Reinstatement = NA_character_, Premium = 0) {
  if (is.character(x)) {
